...
import static bret.worldexporter.Quad.distanceSq;
...
public class MeshOptimizer {
    ...
    private static boolean canTile(Quad quad) {
        ...
        float widthTop = distanceSq(v[0].getPosition(), v[3].getPosition());
        float widthBottom = distanceSq(v[1].getPosition(), v[2].getPosition());
        if (!floatEq(widthBottom, widthTop, ROUND_BITS)) return false;
        float heightLeft = distanceSq(v[0].getPosition(), v[1].getPosition());
        float heightRight = distanceSq(v[2].getPosition(), v[3].getPosition());
    ...
    private static Comparator<Quad> getQuadComparator() {
        return (quad1, quad2) -> {
            ...
            float heightSq1 = distanceSq(pos1, quad1.getVertices()[1].getPosition());
            float heightSq2 = distanceSq(pos2, quad2.getVertices()[1].getPosition());
            if (heightSq1 != heightSq2) return Float.compare(heightSq2, heightSq1);
            float widthSq1 = distanceSq(pos1, quad1.getVertices()[3].getPosition());
            float widthSq2 = distanceSq(pos2, quad2.getVertices()[3].getPosition());
    ...
    private List<Edge> getEdges(Quad quad) {
        ...
        if (minUMinV == -1 || minUMaxV == -1 || maxUMinV == -1 || maxUMaxV == -1) {
            WorldExporter.LOGGER.warn("Invalid UV state in getEdges");
        ...
        if (canTileU && canTileV) {
            ...
            WorldExporter.LOGGER.warn("getEdges called on a quad that can't be tiled during mesh optimization.");
            return Collections.emptyList();
        }
    }
